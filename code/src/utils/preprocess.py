...
from nl.utilsnl import Laplacian_Nonlinear_torch
...
def geometric_dataset_sparse(q, K, m,  root='../dataset/data/tmp/', subset='Cornell', dataset=WebKB, 
    ...
    if load_only:
        return dataset, X, E, label, train_mask, val_mask, test_mask
    
    try:
        L = hermitian_decomp_sparse(X, m,E,f_node, e_node, size, q, norm=True, laplacian=laplacian, 
    ...
    multi_order_laplacian = cheb_poly_sparse(L, K)
    return dataset, X, label, train_mask, val_mask, test_mask, multi_order_laplacian